-- Migration: 20250618050000_comprehensive_fix.sql
-- Description: Comprehensive fix for all pet profile onboarding issues:
--  1. Function signature with TEXT parameters for medications and supplements
--  2. Fixed ambiguous pet_identifier column reference
--  3. Proper insertion into separate lifestyle tables
--  4. Error handling and defensive programming
--  5. SECURITY DEFINER to solve permission issues
...
DROP FUNCTION IF EXISTS public.create_complete_pet_profile(
    UUID, TEXT, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB
);
DROP FUNCTION IF EXISTS public.create_complete_pet_profile(
    UUID, TEXT, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
);

-- Now create the single, comprehensive function
CREATE OR REPLACE FUNCTION public.create_complete_pet_profile(
    ...
    p_allergies TEXT,
    p_insurance TEXT,
    p_medications TEXT, -- TEXT parameter to accept JSON string from Edge Function
    p_supplements TEXT  -- TEXT parameter to accept JSON string from Edge Function
) RETURNS JSONB AS $$
DECLARE
    profile_id UUID;
    pet_id UUID;
    pet_identifier_val TEXT;
    parsed_meds JSONB;
    parsed_supps JSONB;
    meds_array TEXT[] := '{}';
...
BEGIN
    ...
    BEGIN
        INSERT INTO pets (
            ...
            p_photo_url,
            p_user_id
        ) RETURNING id, pets.pet_identifier INTO pet_id, pet_identifier_val;
    ...
    IF pet_identifier_val IS NULL THEN
        RETURN jsonb_build_object('success', false, 'error', 'Pet created but identifier not generated');
    END IF;

    RETURN jsonb_build_object('success', true, 'pet_id', pet_identifier_val);
END;
