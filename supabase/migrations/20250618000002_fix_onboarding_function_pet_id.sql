...
DROP FUNCTION IF EXISTS create_complete_pet_profile;

-- Recreate the function with improved pet_id handling
...
BEGIN
  ...
  BEGIN
    ...
    IF jsonb_array_length(medications) > 0 THEN
    ...
    IF jsonb_array_length(supplements) > 0 THEN
    ...
    RAISE NOTICE 'Function returning pet_id: %', new_pet_id;

    -- Check if new_pet_id is valid before returning
    IF new_pet_id IS NULL THEN
      RAISE EXCEPTION 'Pet ID generation failed';
    END IF;

    -- Return success with created IDs
    -- NOTE: Edge Function expects 'pet_id' key exactly
