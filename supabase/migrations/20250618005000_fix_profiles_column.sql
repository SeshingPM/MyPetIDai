-- Fix the column name mismatch in the create_complete_pet_profile function
-- The function is using user_id for profiles table but it should be using id
...
BEGIN
  ...
  BEGIN
    ...
    -- FIXED: Changed user_id to id in profiles table
    INSERT INTO profiles (
      id, -- This is the primary key, not user_id
      full_name, 
      zip_code, 
      phone, 
      sms_opt_in,
    ...
    VALUES (
      user_id, -- Using the user_id parameter as the profiles.id 
      owner_full_name, 
      owner_zip_code, 
      owner_phone, 
      owner_sms_opt_in,
    ...
    INSERT INTO pets (
      id,
      user_id, -- Using user_id param here as well (pets.user_id foreign key links to profiles.id)
      name,
      type,
      breed,
      gender,
    ...
    -- Add type checking to prevent "cannot get array length of a scalar" error
    IF medications IS NOT NULL AND jsonb_typeof(medications) = 'array' AND jsonb_array_length(medications) > 0 THEN
    ...
    -- Add type checking to prevent "cannot get array length of a scalar" error
    IF supplements IS NOT NULL AND jsonb_typeof(supplements) = 'array' AND jsonb_array_length(supplements) > 0 THEN
    ...
    -- Return success with created IDs and pet_identifier
